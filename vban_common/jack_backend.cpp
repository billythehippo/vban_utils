...
int parse_passport(const char* input, uint32_t* ip, uint16_t* port)
{
    ...
    struct in_addr xa;
    char garbage;
    if (sscanf(input, "%15[^:]:%d%c", xip_buff, &xport_val, &garbage)) len = 6;
    if (xport_val < 0 || xport_val > 65535) len = 0;
    if (inet_pton(AF_INET, xip_buff, &xa)!= 1) len = 0;
    if (len == 6)
    {
        *ip = xa.s_addr;
...
int get_emitter_options(vban_stream_context_t* stream, int argc, char *argv[])
{
    ...
    while(c!=-1)
    {
        switch (c)
        {
            ...
            case 'd': // Device mode in graph
                ...
                break;
            case 'a':
                if (((stream->flags&DEVICE_MODE)!= DEVICE_MODE)&&(optarg[0]=='y')) stream->flags|= AUTOCONNECT;
                break;
            case 'f':
    ...
    {
        if (nbcd) stream->nboutputs = nbcd;
        else stream->nboutputs = stream->nbinputs;
    }
    return 0;
}
...
int get_receptor_options(vban_stream_context_t* stream, int argc, char *argv[])
{
    ...
    while(c!=-1)
    {
        ...
        case 'b':
            stream->flags|= DUPLEX;
            if (optarg[0]==0) stream->nbinputs = atoi(optarg);
            break;
        case 'h':
            help_receptor();
            return 1;
...
int on_tx_process(jack_nframes_t nframes, void *arg)
{
    ...
    if (event_count>0)
    {
        ...
        else
        {
			...
			if (context->pipedesc == 1) fflush(stdout);
        }
