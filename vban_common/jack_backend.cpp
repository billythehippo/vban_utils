...
void help_emitter(void)
{
    ...
    fprintf(stderr, "-a - enable autoconnect (not in device mode)\r\n");
    fprintf(stderr, "-b - bidirectional mode (run receptor)");
...
void help_receptor(void)
{
    ...
    fprintf(stderr, "-a - enable autoconnect (not in device mode)\r\n");
    fprintf(stderr, "-b - bidirectional mode (run emitter)");
    fprintf(stderr, "-h - show this help\r\n");
    exit(0);
}
...
int get_emitter_options(vban_stream_context_t* stream, int argc, char* argv[])
{
    ...
    static const struct option options[] = {
        ...
        { "bidirection", required_argument, 0, 'b' },
        { "extended", required_argument, 0, 'x' },
        { "help", no_argument, 0, 'h' },
        { 0, 0, 0, 0 }
    };

    c = getopt_long(argc, argv, "m:i:p:s:r:q:c:n:d:a:f:e:j:b:x:h", options, &index);
    if (c == -1)
        c = 'h';
    ...
    while (c != -1) {
        ...
        case 'a':
            ...
            break;
        case 'f':
            if (strstr(optarg, "16") != NULL)
                stream->vban_output_format = VBAN_BITFMT_16_INT;
            else if (strstr(optarg, "24") != NULL)
                stream->vban_output_format = VBAN_BITFMT_24_INT;
        ...
        case 'b':
            ...
            break;
        case 'x':
            stream->xlen = parse_passport(optarg, &stream->xaddress.xip, &stream->xaddress.xport);
            break;
        case 'h':
            help_emitter();
            return 1;
    ...
    if ((stream->flags & DUPLEX) != 0) {
        if (nbcd)
            stream->nboutputs = nbcd;
        else
            stream->nboutputs = stream->nbinputs;
    }
    return 0;
}
...
int get_receptor_options(vban_stream_context_t* stream, int argc, char* argv[])
{
    ...
    c = getopt_long(argc, argv, "m:i:p:s:r:q:c:n:d:a:f:e:j:b:h", options, &index);
    if (c == -1)
        c = 'h';
...
int jack_init_tx_stream(jack_stream_data_t* client)
{
    ...
    if ((stream->flags & DEVICE_MODE) != 0)
        port_flags |= JackPortIsPhysical;
    if (stream->jack_server_name != nullptr)
        client->options = JackServerName;
...
int jack_init_rx_stream(jack_stream_data_t* client)
{
    ...
    if ((stream->flags & DEVICE_MODE) != 0)
        port_flags |= JackPortIsPhysical;
    if ((stream->jack_server_name != NULL) && (stream->jack_server_name[0] != 0))
        client->options = JackServerName;
...
int jack_run_tx_stream(jack_stream_data_t* client)
{
    ...
    if ((stream->flags & DEVICE_MODE) != DEVICE_MODE)
        jack_connect_stream(client, stream->nbinputs);
    return ret;
}
...
int jack_run_rx_stream(jack_stream_data_t* client)
{
    ...
    if (ret)
        fprintf(stderr, "Cannot activate client\n");
    if ((stream->flags & DEVICE_MODE) != DEVICE_MODE)
        jack_connect_stream(client, 0, stream->nboutputs);
    return ret;
}
...
void jack_stop_rx_stream(jack_stream_data_t* client)
{
    jack_deactivate(client->client);
    jack_client_close(client->client);
    if (client->output_ports != nullptr)
        free(client->output_ports);
}
        ...
        if (inputs > 0) {
            ...
            if (ports != 0) {
                for (port_id = 0; port_id < 1024; port_id++) {
                    if (ports[port_id] == 0)
                        break;
                    if (strstr(ports[port_id], "midi") == NULL)
                        pports++;
                }
                for (port_id = 0; port_id <= inputs; port_id++) {
                    if (input_matrix != nullptr)
                        port_ind = input_matrix[port_id];
                    else
                        port_ind = port_id;
            ...
            if (ports != 0) {
                ...
                for (port_id = 0; port_id <= outputs; port_id++) {
                    if (output_matrix != nullptr)
                        port_ind = output_matrix[port_id];
                    else
                        port_ind = port_id;
...
int on_tx_process(jack_nframes_t nframes, void* arg)
{
    ...
    // if ((context->txport == 0)&&(context->pipedesc == 1)) fflush(stdout);
    return 0;
}
...
int on_rx_process(jack_nframes_t nframes, void* arg)
{
    ...
    if (context->flags & CORRECTION_ON)
        correct_samplerate(context, framesize, &data->callback_delta);
