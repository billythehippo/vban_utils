...
int get_emitter_options(vban_stream_context_t* stream, int argc, char* argv[])
{
    ...
    c = getopt_long(argc, argv, "m:i:p:s:r:q:c:n:d:f:e:b:x:h", options, &index);
    if (c == -1)
        c = 'h';
...
int get_receptor_options(vban_stream_context_t* stream, int argc, char* argv[])
{
    ...
    c = getopt_long(argc, argv, "m:i:p:s:r:q:c:n:d:f:e:b:h", options, &index);
    if (c == -1)
        c = 'h';
...
static void on_rx_process(void* userdata)
{
    ...
    if (context->lost_pac_cnt < 9)
        memcpy(samples_ptr, (char*)context->rxbuf, context->rxbuflen * sizeof(float));
    else
        memset(samples_ptr, 0, context->rxbuflen * sizeof(float));
...
int pw_init_tx_stream(pw_stream_data_t* data, struct pw_stream_events* stream_events, vban_stream_context_t* vban_stream, spa_audio_format format)
{
    ...
    if (vban_stream->txbuf != NULL)
        free(vban_stream->txbuf);
