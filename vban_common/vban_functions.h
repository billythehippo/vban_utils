...
inline uint vban_strip_vban_packet(uint8_t format_bit, uint16_t nbchannels)
{
    ...
    if (nframes > VBAN_SAMPLES_MAX_NB)
        nframes = VBAN_SAMPLES_MAX_NB;
...
{
    if ((deltastruct->ts_new.tv_sec == deltastruct->ts_old.tv_sec) && (deltastruct->ts_new.tv_nsec == deltastruct->ts_old.tv_nsec))
        clock_gettime(CLOCK_MONOTONIC_RAW, &deltastruct->ts_new);
    if (order > 5)
        order = 5;
    if (deltastruct->ts_old.tv_sec) {
        ...
        if (deltastruct->deltatf[order - 1] == 0) {
            ...
            for (int f = 1; f < order; f++)
                deltastruct->deltatf[f] = deltastruct->deltatf[0];
        } else {
            deltastruct->deltatf[0] = (1 - weight) * deltastruct->deltatf[0] + weight * (double)deltastruct->deltat;
            for (int f = 1; f < order; f++)
                deltastruct->deltatf[f] = (1 - weight) * deltastruct->deltatf[f] + weight * deltastruct->deltatf[f - 1];
        }
    }
...
inline void calc_ratio_correction(vban_stream_context_t* context, timestamp_delta* callback_delta, int framesize, int rbfill_target)
{
    ...
    if (abs(context->rratiof - 1) > 0.0025)
        context->rratiof = 1;
    float w = 0.01;
    int thrsh = (context->nframes + context->cframes + context->lagrange_num) * 2;
    if (context->rbfill - rbfill_target > thrsh)
        w = 0.1;
    ...
    if (context->rratio > 1.003)
        context->rratio = 1.003;
    else if (context->rratio < 0.998)
        context->rratio = 0.998;
...
inline int vban_send_txbuffer(vban_stream_context_t* context, in_addr_t txip = 0, uint8_t attempts = 2)
{
    int ret = 0;
    uint8_t xlen = 0;
    if ((context->txport == 0 || context->iptx == 0x0100007F) && context->xlen == 6)
        xlen = 6; // ONLY localhost and pipes
    ...
    for (uint16_t pac = 0; pac < context->pacnum; pac++) {
        ...
        if (xlen == 6)
            memcpy(context->txpacket.data + context->pacdatalen, &context->xaddress, context->xlen);

        for (uint8_t red = 0; red <= context->redundancy; red++)
            if (context->txport != 0) {
                udp_send(context->txsock, context->txport, (char*)&context->txpacket, VBAN_HEADER_SIZE + context->pacdatalen + xlen, txip);
#ifdef __linux__
                int icnt = 0;
                usleep(10);
                ...
                if (icnt == attempts)
                    ret = 1; // ICMP PORT UNREACHABLE
#endif
            } else {
                write(context->pipedesc, (uint8_t*)&context->txpacket, VBAN_HEADER_SIZE + context->pacdatalen + context->xlen);
                if (context->pipedesc == 1)
                    fflush(stdout);
            }
    ...
    for (uint16_t pac = 0; pac < context->pacnum_t32; pac++) {
        ...
        for (uint8_t red = 0; red <= context->redundancy; red++)
            if (context->txport != 0) {
                ...
                write(context->pipedesc, (uint8_t*)&context->txpacket, VBAN_HEADER_SIZE + context->pacdatalen);
        context->txpacket.header.nuFrame++;
        // fprintf(stderr, "%d\r\n", txbuf_pos);
    }
...
inline void read_from_ringbuffer_async(vban_stream_context_t* context)
{
        ...
        if (rb_readspace < sizeof(float) * context->nboutputs) {
            ...
            if (llost < 3) //(llost == 1)
            {
                ...
                for (int channel = 0; channel < context->nboutputs; channel++)
                    ((float*)(context->rxbuf + rxbuf_pos))[channel] = 0.8 * 3 * ((float*)(context->rxbuf + rxbuf_pos - context->nboutputs))[channel] - 0.8 * 3 * ((float*)(context->rxbuf + rxbuf_pos - 2 * context->nboutputs))[channel] + 0.8 * ((float*)(context->rxbuf + rxbuf_pos - 3 * context->nboutputs))[channel]; // LAGRANGE
                lframe++;
            }
        } else {
            ...
            if (llost < 3) {
                for (int channel = 0; channel < context->nboutputs; channel++)
                    ((float*)(context->rxbuf + rxbuf_pos))[channel] = 0.3 * ((float*)(context->rxbuf + rxbuf_pos))[channel] + 0.9 * ((float*)(context->rxbuf + rxbuf_pos - context->nboutputs))[channel] - 0.9 * ((float*)(context->rxbuf + rxbuf_pos - 2 * context->nboutputs))[channel] + 0.3 * ((float*)(context->rxbuf + rxbuf_pos - 3 * context->nboutputs))[channel];
            }
    ...
    if (lost == 0) {
        ...
        if (context->lost_pac_cnt < 9)
            fprintf(stderr, "%d samples lost\n", lost);
        if (lost == context->nframes) {
            if (context->lost_pac_cnt < 10)
                context->lost_pac_cnt++;
            if (context->lost_pac_cnt == 9)
...
inline int calc_input_samplerate(vban_stream_context_t* context, VBanPacket* vban_packet)
{
    ...
    if (timedeltatmp > 100000) {
        ...
        if (vban_packet->header.nuFrame != context->nu_frame + 1)
            context->flags |= NET_XRUN;
        context->cycle_pac_cnt += 1;
        context->cycle_frames += vban_packet->header.format_nbs + 1;
        ret = 1;
...
inline void cmd_present_send(vban_stream_context_t* context)
{
...
inline static int vban_rx_handle_packet(VBanPacket* vban_packet, int packetlen, vban_stream_context_t* context, uint32_t ip_in, uint16_t port_in)
{
    int bframe = 0;
    int iframe = 0;
    int frame = 0;
    ...
    case VBAN_PROTOCOL_AUDIO:
        ...
        {
            if (context->nboutputs == 0)
                context->nboutputs = vban_packet->header.format_nbc + 1;
            ...
            (context->iprx == ip_in)) //&&(ip_in!= iplocal))
        {
            ...
            if (context->resampler_inbuflen < (vban_packet->header.format_nbs + 1)) {
                if (context->resampler_inbuf != nullptr)
                    free(context->resampler_inbuf);
                if (context->resampler_outbuf != nullptr)
                    free(context->resampler_outbuf);
    ...
    case VBAN_PROTOCOL_SERIAL:
        if ((vban_packet->header.vban == VBAN_HEADER_FOURC) && (vban_packet->header.format_SR == 0x2E) && ((strcmp(vban_packet->header.streamname, context->rx_streamname) == 0) || (strcmp(vban_packet->header.streamname, "MIDI1") == 0))) {
