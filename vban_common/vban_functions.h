...
inline int vban_send_txbuffer(vban_stream_context_t* context, in_addr_t txip = 0, uint8_t attempts = 2)
{
    int ret = 0;
    uint8_t xlen = 0;
    if ((context->txport == 0 || context->iptx==0x0100007F) && context->xlen == 6) xlen = 6;// ONLY localhost and pipes
    ...
    for (uint16_t pac = 0; pac < context->pacnum; pac++)
    {
        ...
        if (xlen == 6) memcpy(context->txpacket.data + context->pacdatalen, &context->xaddress, context->xlen);
